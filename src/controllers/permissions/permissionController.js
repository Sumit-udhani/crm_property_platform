  ...
  try {
    ...
    if (!req.isSuperAdmin && !isOrgAdmin) {
      return res.status(403).json({
        success: false,
        message: "Not allowed to assign permissions",
      });
    }
    ...
    if (!role_id || !Array.isArray(permissions)) {
      return res.status(400).json({
        success: false,
        message: "role_id and permissions required",
      });
    }

    const roleId = BigInt(role_id);

    await prisma.permissions_role.deleteMany({
...
exports.getPermissionsByRole = async (req, res) => {
  try {
    ...
    if (!req.isSuperAdmin && !isOrgAdmin) {
      return res.status(403).json({
        success: false,
        message: "Not allowed to view permissions",
      });
    }
